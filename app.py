...
def plot_family_pie(df: pd.DataFrame, title: str):
    ...
    # keep chart compact and standard-sized
...
st.caption("Store-level recommendations, trends, and item forecasting.")
...
with st.sidebar:
    ...
    store_choice = st.selectbox("Store", stores)
    ...
    item_choice = st.selectbox("Item (for forecast plot)", item_options, index=0)
    ...
    top_n = st.slider("Top N recommendations", min_value=5, max_value=20, value=10)
    show_forecast_details = st.checkbox("Show forecast details table", value=True)
...
with tab1:
    st.subheader(f"{view_mode} recommendations for Store {store_choice}")

    if view_mode == "Replenish":
        with st.container(border=True):
            chart_left, chart_center, chart_right = st.columns([1, 2, 1])
            with chart_center:
                st.markdown(f"#### Family distribution (Top {top_n})")
